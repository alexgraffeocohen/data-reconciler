		...
		@dbp_data.each do |dbp_term, dbp_term_stripped|
			...
			#  		@stem_matches_csv << [dbp_term, @iptc_data.key(dbp_term_stripped)]
				...
				@iptc_data.each do |iptc_term, iptc_term_stripped|
					...
					if dbp_term_stripped == iptc_term_stripped  #matches if stemmed terms are the same and if the original terms are >= 90% similar
					...
					if dbp_term_set.length == iptc_term_set.length and dbp_in_iptc and iptc_in_dbp and @jarrow.getDistance(dbp_term, iptc_term) >= 0.90
